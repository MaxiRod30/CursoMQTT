...
$conn = mysqli_connect("localhost","admin_iotdarkDB","Q1w2e3r4t5","admin_iotdarkDB");



$devicesSeriearray = array();

foreach ($devices as $device ) {
  array_push($devicesSeriearray,$device['devices_serie']);
}
      ...
      <div ui-view class="app-body" id="view">
        ...
        <div class="padding">
          ...
          <div class="row">
            ...
            <div class="col-xs-12 col-sm-9">
              <div class="box p-a">
              <div class="form-group row" style="margin-left:25px">CONTROL ARDUINO 1</div>
                <div class="form-group row">
                  <button onclick="command('ON',<?php echo  $devicesSeriearray[0]?>,'ON_OUT1')" class="md-btn md-raised m-b-sm w-xs green" style="margin-left:25px">Enceder</button>
                  <button onclick="command('OFF',<?php echo $devicesSeriearray[0]?>,'OFF_OUT1')" class="md-btn md-raised m-b-sm w-xs red"  style="margin-left:25px" >Apagar</button>
                </div>
                <div class="form-group row">

                  <button onclick="command('ON',<?php echo  $devicesSeriearray[0]?>,'ON_OUT2')" class="md-btn md-raised m-b-sm w-xs green" style="margin-left:25px">Enceder</button>
                  <button onclick="command('OFF',<?php echo $devicesSeriearray[0]?>,'OFF_OUT2')" class="md-btn md-raised m-b-sm w-xs red"  style="margin-left:25px" >Apagar</button>

                </div>
                <div class="form-group row">

                  <button onclick="command('ON',<?php echo  $devicesSeriearray[0]?>,'ON_OUT3')" class="md-btn md-raised m-b-sm w-xs green" style="margin-left:25px">Enceder</button>
                  <button onclick="command('OFF',<?php echo $devicesSeriearray[0]?>,'OFF_OUT3')" class="md-btn md-raised m-b-sm w-xs red"  style="margin-left:25px" >Apagar</button>
        ...
        <div class="box">
          <div class="box-header">
            ...
            <small>Visualizacion de grafico</small>
          </div>
          <div class="box-body">
            <div ui-jp="plot" ui-refresh="app.setting.color" ui-options="
              [
                { 
                  data: [[1, 5.5], [2, 5.7], [3, 6.4], [4, 7.0], [5, 7.2], [6, 7.3], [100, 7.5]], 
...
function command(action, device_serie,output){
  console.log(device_serie);
  if(action == "ON"){
    client.publish(device_serie + "/command", output , (error) => {
      console.log(error || 'Encediendo'+ output +' ' + device_serie)
    });
  }else{
    client.publish(device_serie + "/command", output, (error) => {
      console.log(error || 'Apagando '+ output +' '+ device_serie)
    });
  }
}
...
const WebSocket_URL = 'wss://iotdarkmaxvs.ga:8094/mqtt'
...
client.on('connect', () => {
    ...
    <?php foreach ($devices as $device) { ?>
      ...
      client.subscribe('<?php echo $device['devices_serie'] ?>/temps', { qos: 0 }, (error) => {})
    <?php } ?>
...
function prueba(){
  ...
  xhttp.open('GET','Pgrafic.php',true);
  xhttp.send();
  ...
  xhttp.onreadystatechange = function(){
    if(this.readyState == 4 && this.status == 200){
      ...
      //Transformo en objeto y despues busco lo que quiero meditante el objeto ITEM
